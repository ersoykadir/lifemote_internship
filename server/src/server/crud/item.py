...
class Item:
    ...
    def get_item(self, database: Session, item_id: int):
        """Get item by id"""
        db_item = (
            database.query(item_model.Item)
    ...
    def get_items_by_user(self, database: Session, user_id: int):
        ...
        return (
            database.query(item_model.Item)
            .filter(item_model.Item.owner_id == user_id)
            .all()
        )

    def get_items_by_context_for_user(
        self, database: Session, user_id: int, context_id: int
    ):
